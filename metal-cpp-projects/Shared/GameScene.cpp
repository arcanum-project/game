...
: _pDevice(pDevice),
  ...
  _pCamera(std::make_unique<IsometricCamera>()) {
	...
	_pCamera->setRotation(glm::vec3(glm::radians(35.26f), glm::radians(225.0f), .0f));
}
...
const std::shared_ptr<const Model> GameScene::importTile(MTL::Device * const pDevice) {
  ...
  return std::make_shared<const Model>(imported->vertexData, imported->indices, pDevice, RenderingConstants::NumOfTilesPerSector, RenderingConstants::MaxBuffersInFlight, "GrsBse0a_0", "bmp");
}
